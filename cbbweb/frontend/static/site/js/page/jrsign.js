...
$(function () {
    ...
    var series_id = CBB.getUrlParam("series_id"),
        ...
        loan_plan = CBB.getUrlParam("loan_plan"),
        brand_id = $("#default-data").attr("data-brand"),
        sku_item=CBB.getUrlParam("sku"),
        percent=CBB.getUrlParam("percent");
        ...
        switch (li_type) {
            case "li-area-pv":
                ...
                if($(this).parent().prev().attr("id")=="dealer-address-pv"){
                    ...
                    $("#dealer-address-ar .s-main").text("选择县区");
                }else{
                    $("#address-ct-div .s-main,#dealer-address-ct .s-main").text("选择城市");
                    $("#address-ar-div .s-main,#dealer-address-ar .s-main").text("选择县区");
                }
...
//      });
    ...
    $("#pop-dealer-ul").delegate("li", "click", function () {
        ...
        if ($(this).find('.st').hasClass('z-active')) {
            ...
            emptyArea();
            $("#pop-dealer-ul .st").removeClass("z-active");
...
//提交数据
    $('.j-submit').on('click', function () {
        ...
        var _th = this,
            ...
            dealers_code = $("#selected-dealer li").eq(0).attr("xg-zyd-index"),
...
//计划
    ...
    // getPriceByDlMd
    function getLoanPlanDt(Dealer) {
        var model_id = $("#product_id").attr("data-model"),
            series_id = $("#carSeriesId").val(),
            city_id = $("#dealer-address-ct").attr("data-ctid"),
            model_id = $("#product_id").attr("data-model"),
            loan_plan = CBB.getUrlParam("loan_plan"),
            sku= CBB.getUrlParam("sku"),
            percent= CBB.getUrlParam("percent");
        var price_wrap = $(".price-price-r .s-nm");
        price_wrap.html("");//清空原有报价，待后面更新
        if (!Dealer.length ) {
            Dealer = $("#selected-dealer li").eq(0).attr("data-dealerid");
        }else{
            Dealer=Dealer[0].dealer_id;
        }
        if (Dealer && model_id && series_id && sku && percent && city_id) {
            var url=$("#default-data").attr("data-planulr");
            $.ajax({
                url: url,
                data: {
                    'dealer_id': Dealer,
                    'car_series_id': series_id,
                    'car_type_id': model_id,
                    'city_id': city_id,
                    'finacial_product_id':loan_plan,
                    'sku_item':sku,
                    'first_pay_percent':percent
                },
                success: function (data) {
    ...
    function refsDlPrice(obj) {
        $("#loan-plan-brand").text(obj.corp_name);
        $("#loan-plan-name").text(obj.financial_product_name);
        $("#laon-product-first-pay").text((obj.first_pay_amount/10000).toFixed(2) + "万("+obj.first_pay_percent+"%)")
        $("#laon-product-sku-item").text(obj.sku_name)
        var price_wrap = $(".price-price-r li");
        if (obj) {
            price_wrap.eq(0).find(".s-nm").text((obj.loan_amount / 10000).toFixed(2) + "万")
            price_wrap.eq(1).find(".s-nm").text(obj.monthly_pay_amount  + "元")
            price_wrap.eq(2).find(".s-nm").text(obj.loan_cost >= 10000 ? (obj.loan_cost / 10000).toFixed(2) + "万" : obj.loan_cost + "元");
        } else {
            price_wrap.eq(0).find(".s-nm").text("暂无")
            price_wrap.eq(1).find(".s-nm").text("暂无")
            price_wrap.eq(2).find(".s-nm").text("暂无");
        }
